...
#endif//Moto_UI      
#ifdef MESHCOM_UI
    byte dataReaded = 0;bool stateS = CONFIG::read_byte (EP_EEPROM_WIFI_MODE, &dataReaded);
...
GyLog.Loop();
  ...
  if(Buttonfunction == 1) {State = !State;GyroWifiOn = false;Buttonfunction = 10;
  #ifdef FC_Gyro
    if(State){ GyLog.Start_PPM(); 
        CONFIG::write_byte(GyroState,1);Serial.println("Run");}
    else{CONFIG::write_byte(GyroState,0);digitalWrite(LEDstt,HIGH);Serial.println("Stop");}
  #else
    if(State){CONFIG::write_byte(GyroState,1);Gyros.Start();Serial.println("Run");}
    else{CONFIG::write_byte(GyroState,0);digitalWrite(LEDstt,HIGH);Serial.println("Stop");}
  #endif//FC_Gyro
  }
    // slow blink (must hold down button. 1 second long blinks)
  if(Buttonfunction == -1){GyroWifiOn = true;Serial.println("Wifi On");Buttonfunction = 10;
