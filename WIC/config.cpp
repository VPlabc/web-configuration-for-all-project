...
byte CONFIG::GyroStates = DEFAULT_GYRO_STATE; 
#endif//gyro_UI
#ifdef Valve_UI
#include "Valve/Valve.h"
#include "Valve/SDFunction.h"
#endif//Valve_UI
...
const char* CONFIG::GetFirmwareTargetShortName()
{
    ...
    if ( CONFIG::FirmwareTarget == REPETIER4DV) {
        ...
        response = F ("grbl");
    } else if ( CONFIG::FirmwareTarget == LOOKLINE) {
        response = F ("lookline");
    } else if ( CONFIG::FirmwareTarget == LOOKLINEGW) {
        response = F ("looklineGw");
    } else if ( CONFIG::FirmwareTarget == VALVEGW) {
        response = F ("valveGw");
    } else if ( CONFIG::FirmwareTarget == LIGHT_TM) {
        response = F ("switchTimer");
    } else if ( CONFIG::FirmwareTarget == TESTING) {
        response = F ("circuitTest");
    } else if ( CONFIG::FirmwareTarget == GYRODATALOG) {
        response = F ("gyroDatalog");
    } else if ( CONFIG::FirmwareTarget == RF_HUB) {
        response = F ("rfHub");
    }  else if ( CONFIG::FirmwareTarget == MESH_HUB) {
        response = F ("meshHub");
    }  else if ( CONFIG::FirmwareTarget == IOT_DEVICES) {
        response = F ("iotDevice");
    }  else if ( CONFIG::FirmwareTarget == MOTO_DASH) {
        response = F ("motoDash");
    } else {
        response = F ("???");
    }
...
{
    CONFIG::GyroStates = false;
    if (!CONFIG::read_byte (GyroState, &CONFIG::GyroStates ) ) {
        CONFIG::GyroStates = DEFAULT_GYRO_STATE;
    }
 return CONFIG::GyroStates;
}
#endif//Gyro_UI
...
void CONFIG::InitDirectSD()
{
        ...
        if (!SD.begin(SDCard_CS)) {
            ...
            #ifdef Valve_UI
            Valve::showInfo("SD Card", "Not Found", 3);
            ...
            #ifdef Valve_UI
            Valve::showInfo("SD Card", "Found", 3);
            //Valve::loadSavedSensors();
