...
#include <SoftwareSerial.h>
 SoftwareSerial serial_ESP(4,5);
#define ModbusSerial serial_ESP
#endif//ESP32
                      ...
                      unsigned int* _regs)
{
	...
	modbus_update_slave_comms(bauds, _slaveID);
...
void modbus_update_slave_comms(long bauds, unsigned char _slaveID)
{
	
	#ifdef ESP32
		ModbusSerial.begin(bauds, SERIAL_8N1);
	#else
		// ModbusSerial.begin(baud, SWSERIAL_8N1, D2, D3, false, 95, 11);
		// ModbusSerial(D2, D3, false, 64, 0);
		ModbusSerial.begin(bauds);
	#endif//ESP32
	slaveID = _slaveID;
  ...
  // 1.5T = 1.04167ms * 1.5 = 1.5625ms. A frame delay is 3.5T.
	...
	{
		T1_5 = 15000000/bauds; // 1T * 1.5 = T1.5
		T3_5 = 35000000/bauds; // 1T * 3.5 = T3.5
	}
}  
